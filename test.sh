...
BUCKET=$data-engineer-357919-dsongcp
YEAR=$2015
...
#bq --project_id $PROJECT rm -f ${PROJECT}:dcongcp.flights_raw
bq --project_id $PROJECT show dcongcp || bq mk --sync dcongcp
...
bq --project_id $PROJECT  --sync \
   ...
   --replace ${PROJECT}:dcongcp.flights_raw\$${YEAR}${MONTH} $CSVFILE
